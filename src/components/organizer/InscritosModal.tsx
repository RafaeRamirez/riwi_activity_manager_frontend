...
export function InscritosModal({ evento, onClose, onReload }: InscritosModalProps) {
  ...
  const cargarInscritos = async () => {
    try {
      ...
      const inscritosConFechas = data.map((inscrito: any) => ({
        ...inscrito,
        fechaInscripcion: new Date(inscrito.fechaInscripcion)
      }));

      setInscritos(inscritosConFechas);
      console.log('✅ Inscritos cargados:', inscritosConFechas.length);
    } catch (error: any) {
  ...
  const handleDesinscribir = async (inscritoId: string, nombreCoder: string) => {
    ...
    try {
      console.log('🔄 Desinscribiendo coder:', inscritoId);
      ...
      // Recargar lista de eventos si se proporciona la función
      if (onReload) {
        onReload();
      }
      ...
      alert('Error al desinscribir al coder.');
    }
  };
                  ...
                  {inscritos.map((inscrito) => (
                    ...
                    >
                      <div className="flex items-center gap-3 flex-1">
                        {/* Avatar */}
                        <div className="w-12 h-12 rounded-full bg-linear-to-br from-riwi-green to-riwi-yellow flex items-center justify-center text-white font-bold flex-shrink-0">
